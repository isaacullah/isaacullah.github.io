...
- [MicMac](http://logiciels.ign.fr/?Micmac) - This is a very cool Open Source project that provides a complete cross-platform SfM pipeline that does NOT rely on Bundler. It seems possible to produce georefrenced point clouds as well. The site is in the French language, but much of the documentation is also in English. This project seems to be actively developed, and it seems that a nice GUI interface is in the works. CLI only for now, though. [Linux, Mac, Windows]
